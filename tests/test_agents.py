...
def test_derive_episodic_metrics_matches_manual_masked_mean():
    ...
    for s in range(n_seeds):
        for u in range(n_updates):
            ...
            assert np.allclose(derived["episode/length"][s, u], expected_length)
            assert np.allclose(derived["episode/returns"][s, u], expected_returns)
            assert np.allclose(derived["episode/success_rate"][s, u], expected_success)

    # original logs dict must not be mutated
    assert "episode/returns" not in logs


def test_derive_episodic_metrics_raises_on_missing_keys():
    logs = {"train/done_mask": jnp.ones((2,), dtype=bool)}
...
def test_log_to_wandb_on_train_end_respects_log_frequency():
    ...
    # where train/updates % log_frequency == 0.
    n_steps = 10
    log_frequency = 3
    logs = {
        "train/updates": jnp.arange(n_steps),
        "train/frames": jnp.arange(n_steps) * 100,
    }
...
def test_log_to_wandb_masked_mean_matches_boolean_indexing():
    ...
    logs = {
        "train/updates": jnp.asarray(0),
        "train/frames": jnp.asarray(0),
        "train/done_mask": jnp.asarray(mask),
        "train/lengths": jnp.asarray(lengths),
        "train/returns": jnp.asarray(returns),
    }
    ...
    assert np.allclose(logged["episode/length"], expected_length)
    assert np.allclose(logged["episode/returns"], expected_returns)
    assert np.allclose(logged["episode/success_rate"], expected_success_rate)
