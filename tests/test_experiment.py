...
class _FakeAgent(Agent):
    ...
    def train(self, rng):
        n_updates = 3
        logs = {
            "train/updates": jnp.arange(n_updates),
            "train/frames": jnp.arange(n_updates) * 100,
        }
        return None, logs
...
class _QuadraticFitnessAgent(Agent):
    """A minimal Agent whose fitness (episode/returns, via
    ...
    def train(self, rng):
        ...
        logs = {
            "train/updates": jnp.arange(n_updates),
            "train/frames": jnp.arange(n_updates) * 100,
            "train/done_mask": jnp.ones((n_updates, num_steps, num_envs), dtype=bool),
            "train/returns": jnp.full((n_updates, num_steps, num_envs), fitness),
            "train/lengths": jnp.ones((n_updates, num_steps, num_envs)),
        }
        return None, logs
