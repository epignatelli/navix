"""Scores navix's own PPO against the Navix1M benchmark preset, under
...
def train_with_hparams(
    ...
    `PPOEntry.train` (at the real budget) call - only `budget` and
    ...
    Returns:
        ...
        completed episodes), plus every `loss/*` entry PPO's own
        training loop already computes per update as `diagnostics`."""
...
class PPOEntry(AlgorithmEntry):
    """navix's own PPO, wired into `Benchmark`'s `AlgorithmEntry`
