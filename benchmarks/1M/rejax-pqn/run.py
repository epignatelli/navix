"""Scores rejax's PQN against the Navix1M benchmark preset.
...
`RejaxPQNEntry.hparams` before the real run.
...
HPARAMS_DISTR = {
    ...
    "td_lambda": distrax.Uniform(low=0.5, high=0.99),
}
"""rejax.PQN's own continuous hyperparameters - excludes num_envs/
...
def train_with_hparams(hparams: Dict[str, jax.Array], env_id: str, budget: int, rng: jax.Array) -> TrainingCurve:
    """Trains `rejax.PQN` on `env_id` for `budget` timesteps with
    ...
    reduced budget) and `RejaxPQNEntry.train` (at the real budget)
    call - only `budget` and `hparams` differ between the two calls.

    Args:
        hparams (Dict[str, jax.Array]): `rejax.PQN`'s own hyperparameter
            overrides (see `HPARAMS_DISTR` above) - empty uses
            `rejax.PQN`'s own defaults.
    ...
    Returns:
        ...
        own agents' `logs` do)."""
    algo = rejax.PQN.create(env=f"navix/{env_id}", total_timesteps=budget, **hparams)
...
class RejaxPQNEntry(AlgorithmEntry):
    """`rejax`'s PQN, wired into `Benchmark`'s `AlgorithmEntry`
