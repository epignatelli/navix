"""Scores navix's own PPO against the Navix100K benchmark preset.
...
class PPOEntry(AlgorithmEntry):
    def train(self, env_id: str, rng: jax.Array) -> Tuple[Agent, Dict[str, jax.Array]]:
        ...
        agent = PPO(hparams=PPOHparams(budget=self.budget), network=ActorCritic(action_dim=len(env.action_set)), env=env)
