"""Scores rejax's DQN against the Navix100K benchmark preset.
...
`RejaxDQNEntry.hparams` before the real run."""
import sys
...
"""rejax.DQN's own continuous hyperparameters - excludes buffer_size/
fill_buffer/batch_size/target_update_freq/num_epochs (batch-shape/
architecture knobs, not really "tuning" in the same sense), ddqn (a
bool, not continuous), gamma (more a task-horizon property than
...
def train_with_hparams(hparams: Dict[str, float], env_id: str, budget: int, rng: jax.Array) -> TrainingCurve:
    algo = rejax.DQN.create(env=f"navix/{env_id}", total_timesteps=budget, **hparams)
...
@dataclass
class RejaxDQNEntry(AlgorithmEntry):
