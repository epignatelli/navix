"""Scores rejax's SAC against the Navix100K benchmark preset.
...
`RejaxSACEntry.hparams` before the real run."""
import sys
...
HPARAMS_DISTR = {
    ...
    "target_entropy_ratio": distrax.Uniform(low=0.5, high=1.0),
}
"""rejax.SAC's own continuous hyperparameters - excludes buffer_size/
fill_buffer/batch_size/target_update_freq/num_epochs/num_critics
(batch-shape/architecture knobs, not really "tuning" in the same
sense) and gamma (more a task-horizon property than something to
search)."""
...
def train_with_hparams(hparams: Dict[str, float], env_id: str, budget: int, rng: jax.Array) -> TrainingCurve:
    algo = rejax.SAC.create(env=f"navix/{env_id}", total_timesteps=budget, **hparams)
...
@dataclass
class RejaxSACEntry(AlgorithmEntry):
