...
def speedup_by_num_steps():
    ...
    # warm up minigrid
    run_minigrid(gym_env_id, 10, 1)
    # warm up navix
    run_navix(ENV_ID, 10, 1)
...
def table_speedup_by_env():
    ...
    with open("speedup_env.json", "r") as f:
        data = json.load(f)

    envs = list(data.keys())
    navix_row = ["NAVIX"]
    minigrid_row = ["MiniGrid"]
    speedup_row = ["Speedup"]

    for env in envs:
        minigrid_mean, minigrid_std, minigrid_stats = compute_stats(
            data[env]["minigrid"]
        )
        navix_mean, navix_std, navix_stats = compute_stats(data[env]["navix_jit_loop"])
...
def table_speedup_by_num_steps():
    ...
    and two rows: one for MiniGrid and one for Navix.
    Saves the table as a string."""
    with open("speedup_num_steps.json", "r") as f:
        data = json.load(f)
    ...
    for step in steps:
        ...
        navix_mean, navix_std, navix_stats = compute_stats(data[step]["navix_jit_loop"])
    ...
    table = "\n".join(table)
    print(table)
    with open("speedup_num_steps.md", "w") as f:
        f.write(table)
